import type { CliBudgetPolicy, CliLiveSnapshot, WorkflowTemplate } from "../schema/cli-budget-types.js"
...
export function estimateWorkflowBudget(
  ...
  for (const [cliTemplateId, usage] of byCli) {
    ...
    if (snapshot?.quota.windows?.length && policy?.reservePercent) {
      ...
      if (weekly?.remainingPercent !== null && weekly?.remainingPercent !== undefined
        && weekly.remainingPercent < policy.reservePercent) {
        blocked = true
